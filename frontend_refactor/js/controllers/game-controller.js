...
const GameController = (function() {
  ...
  function init(config = {}) {
    ...
    callbacks.onGameStart = callbacks.onGameStart || function() {};
    callbacks.onGameEnd = callbacks.onGameEnd || function() {};
    callbacks.onRoundComplete = callbacks.onRoundComplete || function() {};
  ...
  function handleResize() {
    if (gameState.gameActive) {
      // Try to access PongEngine first (new architecture)
      if (typeof PongEngine !== 'undefined') {
        PongEngine.resize();
      }
      // Fall back to PongGame if it exists (old architecture)
      else if (typeof PongGame !== 'undefined') {
        PongGame.resize();
      }
      // If neither exists, just log a warning
      else {
        console.warn("Could not resize game - neither PongEngine nor PongGame is defined");
      }
    }
  }
  ...
  function handleStartGame() {
    ...
    if (selectedMode === "Custom Game") {
      ...
      if (WebSocketService.isConnected()) {
        ...
        PageManager.showAlert(I18nManager.get("connectionError"));
      }
    }
  }
  ...
  function startGame() {
    ...
    setTimeout(() => {
      ...
      if (PongEngine) {
        // Initialize pong engine
        PongEngine.init({
          width: elements.pongCanvas.width,
          height: elements.pongCanvas.height,
          isMultiplayer: gameState.isMultiplayer,
          rounds: gameState.targetRounds,
          onRoundComplete: handleRoundComplete,
          onGameOver: handleGameOver,
          onUpdate: function(state) {
            if (PongRenderer) {
              PongRenderer.render(state, {
                nickname: gameState.nickname,
                controlsDisabled: !gameState.isFullscreen
              });
            }
          }
        });
        
        // Initialize pong renderer
        if (PongRenderer) {
          PongRenderer.init(elements.pongCanvas, {
            colors: {
              ballColor: '#00d4ff',
        ...
        // Fallback to original PongGame if PongEngine not available
        PongGame.init(pongConfig);
        PongGame.start();
      }
  ...
  function startCustomGame(settings) {
    ...
    setTimeout(() => {
      const customConfig = {
        ...
        paddleSizeMultiplier: settings.paddleSize / 100, // Convert to multiplier
      ...
      if (PongEngine) {
        // Initialize pong engine with custom settings
        PongEngine.init({
          width: elements.pongCanvas.width,
          height: elements.pongCanvas.height,
          isMultiplayer: gameState.isMultiplayer,
          ...
          options: {
            initialBallSpeed: settings.ballSpeed,
            paddleSizeMultiplier: settings.paddleSize / 100,
            speedIncrement: settings.speedIncrement,
            gravityEnabled: settings.gravityEnabled,
            bounceRandom: settings.bounceRandom
          },
        ...
        // Initialize pong renderer with custom colors
        if (PongRenderer) {
          PongRenderer.init(elements.pongCanvas, {
            colors: {
              ballColor: settings.ballColor,
  ...
  async function endGame() {
    ...
    if (gameState.isMultiplayer) {
      try {
        if (elements.pongStatus) {
          PageManager.showLoading(elements.pongStatus);
        }
        ...
        if (response.success) {
          ...
          PageManager.showToast(I18nManager.get("failedToRecord"), "error");
        }
      } catch (error) {
  ...
  function handleFullscreenChange(isFullscreen) {
    ...
    // Update control state in PongEngine/PongGame
    if (PongEngine) {
      // Normally PongEngine would handle this internally, 
      // but we want to ensure control state is correct
    } else if (PongGame) {
      PongGame.setControlEnabled(isFullscreen);
    }
  }
  ...
  function validateInput(nickname, rounds) {
    if (!nickname || nickname.trim().length === 0) {
      PageManager.showAlert(I18nManager.get("nicknameRequired"));
      return false;
    }
    ...
    if (!validNickname.test(nickname)) {
      PageManager.showAlert("Nickname must be 1-16 characters with only letters, numbers, underscore or hyphen.");
      return false;
    }
    ...
    if (isNaN(roundsNum) || roundsNum < 1 || roundsNum > 20) {
      PageManager.showAlert("Rounds must be a number between 1 and 20.");
      return false;
    }
  ...
  function updateWaitingList(waitingList) {
    ...
    waitingList.forEach(player => {
      ...
      li.addEventListener('click', () => {
        ...
        if (elements.nicknameInput && elements.nicknameInput.value.trim().length > 0) {
          if (elements.startGameButton && !elements.startGameButton.classList.contains("hidden")) {
            elements.startGameButton.classList.add("pulse");
            setTimeout(() => {
              elements.startGameButton.classList.remove("pulse");
            }, 1500);
          }
        } else if (elements.nicknameInput) {

 ...
 * 
 * All easter eggs are locked until the special command "42KL" is entered
 */
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Easter egg lock state
  let easterEggsUnlocked = false;
  
  // Unlock code
  const UNLOCK_CODE = '42KL';
  
  // Check if easter eggs were previously unlocked in this browser
  try {
    easterEggsUnlocked = localStorage.getItem('easterEggsUnlocked') === 'true';
  } catch (e) {
  ...
  document.addEventListener('keydown', (event) => {
    // Skip if not a valid key event
    if (!event.key) return;
    
    // Track key sequences for unlock code and easter eggs
    const key = event.key.toUpperCase();
    
    // Only record alphanumeric keys
    if (/^[A-Z0-9]$/.test(key)) {
      ...
      if (sequence.includes(UNLOCK_CODE) && !easterEggsUnlocked) {
        unlockEasterEggs();
        return;
      }
      
      // Only process easter eggs if unlocked
    ...
    // Check for 'Z' key press for ambient mode (only if unlocked)
    if (easterEggsUnlocked && event.key.toLowerCase() === 'z') {
      toggleAmbientMode();
      return;
    }
    
    // Allow ESC key to exit ambient mode regardless of unlock state
   ...
   */
  function unlockEasterEggs() {
    easterEggsUnlocked = true;
    
    // Store unlock state in localStorage
    try {
      localStorage.setItem('easterEggsUnlocked', 'true');
    } catch (e) {
      // Ignore localStorage errors
    }
    ...
    // Log easter egg hints to console
    console.log("%c🎮 Solocendence Easter Eggs Unlocked! 🎮", "color: #00d4ff; font-size: 16px; font-weight: bold;");
  ...
  /**
   * Show notification that easter eggs are unlocked
   */
  function showUnlockNotification() {
    ...
    notification.className = 'easter-egg-unlock';
    ...
    notification.innerHTML = `
      ...
      <div style="font-size: 24px; color: #00d4ff; margin-bottom: 15px; font-weight: bold;">Easter Eggs Unlocked!</div>
      <div style="font-size: 16px; color: white; margin-bottom: 15px;">Secret features are now available</div>
      <div style="font-size: 14px; color: #87CEEB; margin-top: 15px; font-style: italic;">Check the console for hints</div>
    `;
  ...
  function toggleAmbientMode(forceState) {
    ...
    if (isAmbientMode) {
      ...
      if (bgCanvas) {
        ...
        if (typeof initBackgroundPong === 'function') {
  ...
  // If easter eggs are already unlocked, show console messages
  if (easterEggsUnlocked) {
    console.log("%c🎮 Solocendence Easter Eggs Active 🎮", "color: #00d4ff; font-size: 16px; font-weight: bold;");
    console.log("%c- Press 'Z' for ambient mode", "color: #87CEEB; font-size: 14px;");
    console.log("%c- Try typing '42'", "color: #87CEEB; font-size: 14px;");
    console.log("%c- Feeling anxious? Type 'panic'", "color: #87CEEB; font-size: 14px;");
    console.log("%c- Want to see the team? Type 'team'", "color: #87CEEB; font-size: 14px;");
    ...
    // Add a small hint in the console about the unlock code
    console.log("%c🔒 Easter eggs are locked. Find the secret code to unlock them.", "color: #87CEEB; font-size: 14px;");
    console.log("%c💡 Hint: The answer to life, the universe, and everything... with initials of a famous school.", "color: #87CEEB; font-style: italic; font-size: 12px;");
  }
});